<template>
  <section 
    :class="['impact-section py-32 px-6 border-b relative z-20 transition-colors duration-400', isDarkMode ? 'theme-dark' : 'theme-light']"
  >
    <div class="max-w-7xl mx-auto">
      ...
      <div ref="impactGrid" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
        <div v-for="(metric, idx) in impactMetrics" :key="idx" class="impact-card rounded-2xl p-8 text-center space-y-2 relative group transition-colors">
          <div class="text-4xl md:text-5xl font-black tracking-tight impact-counter-text group-hover:text-current transition-colors duration-300">
...
.impact-heading { color: var(--heading-color) !important; transition: color 0.4s ease; }
.impact-card { background: var(--card-bg) !important; border: 1px solid var(--card-border) !important; }
.impact-card:hover { border-color: var(--card-border-hover) !important; }
.impact-card:hover .impact-counter-text { color: var(--accent-color) !important; }
.impact-counter-text { color: var(--counter-color); transition: color 0.4s ease; }
.impact-card-label { color: var(--label-color) !important; transition: color 0.4s ease; }
</style>