  ...
  >
    <div class="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
      ...
      <div class="flex justify-center">
        <div
          :class="[
            'w-64 h-[500px] border-[6px] rounded-[44px] p-2 relative overflow-hidden ring-1 will-change-transform transform-gpu',
          ...
          <div :class="['w-full h-full rounded-[34px] overflow-hidden relative isolate', isDarkMode ? 'bg-neutral-950' : 'bg-neutral-50']">
            ...
            >
              <div class="pt-8 space-y-2 z-10 relative">
                <div :class="['font-mono text-xs tracking-widest uppercase', isDarkMode ? 'text-[#00ffa3]' : 'text-[#f97316]']">Metric Panel</div>
      ...
      <div class="space-y-8">
        ...
        <ul ref="whyList" class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <li v-for="(benefit, idx) in appBenefits" :key="idx" class="flex items-start gap-3 p-2">
            <span :class="[
              'flex-shrink-0 w-5 h-5 rounded-full border flex items-center justify-center mt-1',
              isDarkMode ? 'bg-[#00ffa3]/10 border-[#00ffa3]/20' : 'bg-[#f97316]/10 border-[#f97316]/20'
            ]">
              <svg :class="['w-3 h-3', isDarkMode ? 'text-[#00ffa3]' : 'text-[#f97316]']" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="3">
...
<style scoped>
.isolate { isolation: isolate; -webkit-backface-visibility: hidden; backface-visibility: hidden; }
</style>